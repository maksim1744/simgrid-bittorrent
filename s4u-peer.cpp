...
void Peer::operator()()
{
  ...
  std::cout << "status: " << bitfield_.count() << "/" << bitfield_.size() << std::endl;
...
bool Peer::isInterestedBy(const Connection* remote_peer) const
{
  ...
  return (remote_peer->bitfield & ~bitfield_).any();
}
...
void Peer::updatePiecesCountFromBitfield(const std::bitset<FILE_PIECES>& bitfield)
{
  for (unsigned int i = 0; i < FILE_PIECES; i++)
    if (bitfield.test(i))
      pieces_count[i]++;
...
template<size_t N>
unsigned int Peer::countPieces(const std::bitset<N>& bitfield) const
{
...
void Peer::seed()
{
  ...
  int iters = 0;
  while (simgrid::s4u::Engine::get_clock() < deadline && iters < 100) {
    if (comm_received == nullptr) {
      comm_received = mailbox_->get_async<Message>(&message);
    }
    if (comm_received->test()) {
...
{
  current_pieces.reset(current_piece);
...
bool Peer::hasCompletedPiece(unsigned int piece) const
{
  for (unsigned int i = 0; i < PIECES_BLOCKS; i++)
    if (!bitfield_blocks.test(piece * PIECES_BLOCKS + i))
      return false;
